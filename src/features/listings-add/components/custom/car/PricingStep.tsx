...
import type { CarOffer, CarPricing } from "../../../types";

interface PricingStepProps {
  value: CarPricing | undefined;
  onChange: (value: CarPricing) => void;
  error?: string;
...
function PricingStep({
  ...
  const [showOffers, setShowOffers] = useState(pricing.offers.length > 0);
  ...
  const updatePrice = (
    key: keyof Pick<CarPricing, "low_price" | "medium_price" | "high_price">,
    amount: number
  ) => {
  ...
  const addOffer = () => {
    const newOffer: CarOffer = {
      discount_price: 0,
      start_date: "",
      end_date: "",
    };
    onChange({ ...pricing, offers: [...pricing.offers, newOffer] });
  };

  const updateOffer = (
    index: number,
    field: keyof CarOffer,
    fieldValue: string | number
  ) => {
    const updatedOffers = pricing.offers.map((offer, i) =>
      i === index ? { ...offer, [field]: fieldValue } : offer
    );
    onChange({ ...pricing, offers: updatedOffers });
  };

  const removeOffer = (index: number) => {
    onChange({
      ...pricing,
      offers: pricing.offers.filter((_, i) => i !== index),
    });
  };

  // Calculate commission (13%)
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          {/* Low Demand */}
          <div
            className={`p-4 bg-blue-50 rounded-xl border ${fieldErrors?.low_price ? "border-red-400" : "border-blue-100"}`}
          >
          ...
          {/* Medium Demand */}
          <div
            className={`p-4 bg-amber-50 rounded-xl border ${fieldErrors?.medium_price ? "border-red-400" : "border-amber-100"}`}
          >
          ...
          {/* High Demand */}
          <div
            className={`p-4 bg-red-50 rounded-xl border ${fieldErrors?.high_price ? "border-red-400" : "border-red-100"}`}
          >
      ...
      <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
        ...
        <div className="space-y-2 pt-4 border-t border-gray-200">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-gray-600">Commission (13%)</span>
